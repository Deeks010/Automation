...
class ViralContentCreationTasks:
	def topic_analysis(self, agent, text):
		return Task(
			description=dedent(f"""\
				Find trending searches/topics related to the niche:{text}, in the past 1 month.
	...
	def content_research(self, agent, text):
		return Task(
			description=dedent(f"""\
				Do indepth research of all the trending topics and searches.
				For each trending topics related to - {text}, search for 
					the most authoritative and relevant websites within the {text} niche.
	...
	def create_twitter_posts(self, agent, text):
		return Task(
			description=dedent(f"""\
				First filter out the topics that are related to {text} and remove the ones not related.
				Next, Create 5 Twitter posts related to {text} using the content research done for each of 
